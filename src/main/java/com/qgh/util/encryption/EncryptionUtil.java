package com.qgh.util.encryption;
...
public class EncryptionUtil {
    ...
    public static String dealPassword(String password) throws UnsupportedEncodingException {
      ...
      System.out.println("盐值："+salt);
        ...
        System.out.println(salt.getBytes("utf-8"));
        try {
     ...
     */
    public static String dealPasswordWithSalt(String password, byte[] salt) throws UnsupportedEncodingException, NoSuchAlgorithmException {
     if (password==null){
         throw  new InvalidParameterException("paramerter is null");
     }
         //将两个数组合二为一
        byte[] msg = byteCat(password.getBytes("UTF-8"), salt);
        System.out.println("连接后的数组："+msg);
        //加密后的密码
    ...
    public static byte[] getSalt(String dealedPassword) throws InvalidParameterException {
        ...
        byte[] salt = bs[1];
        System.out.println("解密中的盐值："+salt);
